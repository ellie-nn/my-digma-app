...
android.gradle_dependencies = "dummy\u0027 \n println(\u0027=== [GRADLE INIT] ХАК-СКРИПТ РАБОТАЕТ ВНУТРИ СИСТЕМНОГО BUILD.GRADLE ===\u0027); project.afterEvaluate \u007b android.applicationVariants.all \u007b variant -> variant.processManifestProvider.get().doLast \u007b task -> def manifestDir = task.manifestOutputDirectory.get().asFile; fileTree(dir: manifestDir, include: \u0027**/AndroidManifest.xml\u0027).each \u007b manifestFile -> if (manifestFile.exists()) \u007b def text = manifestFile.text; text = text.replace(\u0027android:versionName=\u00221.0.4\u0022\u0027, \u0027android:versionName=\u00221.0.4.1\u0022\u0027); def serviceTag = \u0027<service android:name=\u0022org.oldschool.digmarecorder.DigmaJavaService\u0022 android:process=\u0022:service\u0022 android:stopWithTask=\u0022false\u0022 android:exported=\u0022false\u0022 />\u0027; text = text.replace(\u0027</application>\u0027, serviceTag + System.lineSeparator() + \u0027</application>\u0027); manifestFile.text = text; println(\u0027=== [GRADLE SUCCESS] МАНЕРТВО ВШИТА ВЕРСИЯ 1.0.4.1 И СЛУЖБА JOOD! ===\u0027); \u007d \u007d \u007d \u007d \u007d; //"
